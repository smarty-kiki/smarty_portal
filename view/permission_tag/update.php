...
<head>
    <meta charset='UTF-8'>
    <title>{{ permission_tag::$entity_display_name }}[{{ $permission_tag->id }}]修改</title>
    <style>
     table {
...
<thead>
    ...
    <tr>
        ...
        <td>
            ...
            <select name='{{ $struct }}'>
                @foreach (permission_tag::$struct_formats[$struct] as $key => $value)
                <option value='{{ $key }}' {{ $key === $permission_tag->{$struct}?'selected':'' }}>{{ $value }}</option>
                @endforeach
            </select>
            @else
            <input type='{{ $type }}' name='{{ $struct }}' value='{{ $permission_tag->{$struct} }}'>
            @endif
        </td>
        @endforeach
        <td>
